...
class cCompetir(threading.Thread):
    ...
    def noVoltear(self, up, down, left, right):
        objFil, objCol = self.__aObjetivo.sDirFilCol()[1:] #devuelve la fila y la columna del objetivo
        actFil, actCol = self.__aTanque.sDirFilCol()[1:] #devuelve fial y columna de mi tanque
...
#                 retroceder = False                
            ...
            sleep(velocidad)
            if self.targetIt(dTn, fTn, cTn): #si uno esta en la mira del otro
